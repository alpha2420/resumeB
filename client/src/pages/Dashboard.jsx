    ...
    <div style={{ backgroundColor: '#FEF9ED', minHeight: '100vh' }}>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4 '>
          <button onClick={() => setShowCreateResume(true)} className='w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border border-dashed group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{ backgroundColor: '#FEF9ED', borderColor: '#D89B88', color: '#8B7355' }}>
        ...
        <hr className='my-6 sm:w-[305px]' style={{ borderColor: '#E8B4A0' }} />
        ...
        {showCreateResume && (
          <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 backdrop-blur z-10 flex items-center justify-center' style={{ backgroundColor: 'rgba(93, 82, 75, 0.7)' }}>
            <div onClick={e => e.stopPropagation()} className='relative border shadow-md rounded-lg w-full max-w-sm p-6' style={{ backgroundColor: '#FEF9ED' }}>
              ...
              <button className='w-full py-2 text-white rounded transition-colors hover:opacity-90' style={{ backgroundColor: '#8B7355' }}>Create Resume</button>
              <XIcon className='absolute top-4 right-4 cursor-pointer transition-colors hover:opacity-70' style={{ color: '#8B7355' }} onClick={() => { setShowCreateResume(false); setTitle('') }} />
            </div>
          </form>
        )
        }
